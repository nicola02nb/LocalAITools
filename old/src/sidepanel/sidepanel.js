import {
  convertTextToHTML,
  convertTextToHTMLTokens,
} from "../lib/llmToHtml.js";
    ...
    for await (const chunk of stream) {
      ...
      for (const token of htmlTokens) {
        ...
        await new Promise((resolve) =>
          setTimeout(resolve, settings.delayForStream),
        );
      }
    }
  } catch (error) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  lmForm.addEventListener("submit", async function (event) {
    ...
    if (settings.streamOutput) {
      ...
      Promise.resolve(LM.prompt(text))
        .then((response) => {
          aiText.removeChild(loader);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .getElementById("summarizer-form")
    .addEventListener("submit", async function (event) {
      ...
      if (settings.streamOutput) {
        ...
        Promise.resolve(SUMMARIZER.summarize(text, context))
          .then((response) => {
            aiText.removeChild(loader);
            aiText.innerHTML = convertTextToHTML(response);
          })
...
document.addEventListener("DOMContentLoaded", function () {
  document
    ...
    .addEventListener("submit", async function (event) {
      ...
      await DETECTOR.init();
      Promise.resolve(DETECTOR.detect(text))
        .then((response) => {
          aiText.removeChild(loader);
